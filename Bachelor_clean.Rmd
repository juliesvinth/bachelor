...
# Normality
shapiro.test(df_w_1$diff) # Normal
shapiro.test(df_w_2$diff) # Normal
shapiro.test(df_w_3$diff) # Not normal
```
...
mean(df_w_1$diff, na.rm = TRUE)
sd(df_w_1$diff, na.rm = TRUE)
...
with(df_overall, shapiro.test(diff[intervention == "label"]))# Not normal
with(df_overall, shapiro.test(diff[intervention == "nudge"]))# Normal
...
df_lab_nug <- df_overall %>%
  filter(!(intervention %in% c("inspiration")))

var.test(diff ~ intervention, data = df_lab_nug)  



df_lab_nug$intervention <- relevel(df_lab_nug$intervention, ref = "nudge")

t.test(diff ~ intervention, alternative = "greater", data = df_lab_nug) # Welch one-tailed two-sample t-test
