...
illustration_test <- df_ref_label %>%
  ...
  ggplot(aes(prod_cat.x, oms, fill = intervention.x))+geom_bar( stat = "summary")+ 
...
coop$prod_cat.x <- as.factor(coop$prod_cat.x)
coop$intervention.x <- as.factor(coop$intervention.x)

m0 <- lm(oms  ~ intervention.x, data = coop, REML = FALSE)
m1 <- lmer(oms  ~ intervention.x + (1|prod_cat.x), data = coop, REML = FALSE)
